...
fn schema_response_conforms_and_returns_opencode_settings_v1() {
    ...
    if let Some(schema_id) = result["schema"].get("$id") {
        assert!(
...
fn unknown_schema_id_returns_contract_error_envelope() {
    ...
    let response = assert_error_response(output, "unsupported", "unknown_schema");
...
fn discovery_models_lists_gpt_variants() {
    ...
    assert!(
        output.status.success(),
        "discovery.models exited {:?}; stderr: {}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
    let response = json_stdout(&output);
...
fn discovery_accounts_maps_shuffled_codex_auth() {
    ...
    assert!(
        output.status.success(),
        "discovery.accounts exited {:?}; stderr: {}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
    let response = json_stdout(&output);
...
fn assert_no_oulipoly_provider_path(value: &Value) {
    if let Some(path) = value.as_str() {
        ...
        assert!(
            !normalized.contains("/crates/oulipoly-provider"),
